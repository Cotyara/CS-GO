...
mixer.init()
mixer.music.load("bolshoj-shlyopa-dailyray-pelmeni-mp3.mp3")
mixer.music.play()
fire_sound = mixer.Sound('da.ogg')
...
for i in range(3):
    enemy = Enemy("ufo.png", randint(0, 330), -5, 65, 65, randint(3,5)) 
    monsters.add(enemy)

for i in range(5):
    aste = Enemy("asteroid.png",randint(0, 430), -5, 65, 65, randint(2,3))
    asteroid.add(aste)
...
while game:
    ...
    if finish != True:
        ...
        for m in sprites_list1:
            aste = Enemy("asteroid.png", randint(0, 630), -5, 65, 65, randint(1,2))
            asteroid.add(aste)
